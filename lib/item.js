...
/**
 * @requires this = getDetails result
 * @returns same object with moto attribute setted, if a property is missing, it is set to ''.
 * 
 */
Item.prototype.getBike = function () {
    // if (self.category == "Motos") {
                
    // }
    if (this.details.attributes) {
        this.bike = {};
        this.assignBikeMapDefault(this.bike, this.details.attributes);
    }
    return this;
};
...
const bikeProperties = [ "cubic_capacity", "mileage", "regdate" ];

/**
 * assign bike properties of a given map of { key : "kkk", value : "vvv" } into the given object
 * initialise missing bike property with ''
 */
Item.prototype.assignBikeMapDefault = function(bikeProperty, sourceMap) {
    bikeProperties.forEach(element => {
        var v = sourceMap.find(el => el.key == element);
        if (v == undefined) {
            v = '';
        } else {
            v = v.value;
        }
        bikeProperty[element] = v;
    });
};
